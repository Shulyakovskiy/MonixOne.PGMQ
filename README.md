...
1. Проверяет SHA-256 встроенного архива до изменений в БД.
2. Создаёт `monixone_queue.infrastructure_metadata`.
3. На чистой БД выполняет базовый upstream-скрипт `pgmq.sql` из архива пакета.
4. На уже инициализированной БД читает package-managed версию из metadata.
5. При несовпадении применяет последовательность upstream-скриптов
   `pgmq--X--Y.sql` до версии, встроенной в пакет.
6. Записывает ожидаемую и установленную версии, статус и время проверки.
...
Ошибка подтверждения после фиксации `completed` не переводит сообщение
в retry/DLQ-ветку: следующая доставка повторит только `pgmq.delete`.
